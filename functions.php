...
function urpro_custmonitorcache($id,$day){
  ...
  $data = ['api_key' => urpro_data("apikey","no"), 'monitors'=>$id, 'custom_uptime_ranges'=>$daytime.'_'.current_time('timestamp'),];
  $time = time() - urpro_data("refresh","no");
  $key = 'cache-'.$day.'-'.$id;
  $check = $wpdb->get_results ( "SELECT * FROM $table_name WHERE ur_key = '$key' AND time > '$time'");
  if(count($check) == 0){
	$curl = curl_init('https://api.uptimerobot.com/v2/getMonitors');
	...
	$curl_errno = curl_errno($curl);
		...
		$response = json_decode($respJSON, true);
			if($curl_errno > 0) { return "connection error"; }
...
function urpro_admin_notice(){
	...
	global $wpdb;
	$table_name = $wpdb->base_prefix . 'urpro';
	$data = $wpdb->get_results ( "SELECT * FROM $table_name WHERE ur_key = 'errormessage'");
...
function urpro_siteid(){
  if(isset($GLOBALS['urpro_glob_siteid'])){
	...
	global $wpdb;
	$table_name = $wpdb->base_prefix . 'urpro';
	$data = $wpdb->get_results ( "SELECT * FROM $table_name WHERE ur_key = 'multisite'");
...
function urpro_data($urkey,$base){
  if(isset($GLOBALS['urpro_glob_data_'.$urkey.$base])){
	...
	$table_name = $wpdb->base_prefix . 'urpro';
	$data = $wpdb->get_results ( "SELECT * FROM $table_name WHERE ur_key = '$urkey' AND siteid = '$siteid' ORDER BY id DESC LIMIT 1");
		if(count($data) == 0){
			return "";
		}elseif(count($data) == 1){
			if($data[0]->ur_value == ""){
...
function urpro_datatime($urkey,$base){
	...
	if($base == "yes"){ $siteid = 0; }else{ $siteid = urpro_siteid(); }
	$table_name = $wpdb->base_prefix . 'urpro';
	$data = $wpdb->get_results ( "SELECT * FROM $table_name WHERE ur_key = '$urkey' AND siteid = '$siteid'");
		if(count($data) == 0){
			return "";
		}elseif(count($data) == 1){
			return $data[0]->time;
...
function urpro_monitorcache($id){
 if(isset($GLOBALS['urpro_glob_cache_'.$id])){
  ...
  $data = ['api_key' => urpro_data("apikey","no"), 'response_times'=>'1', 'logs'=>'1', 'monitors'=>$id, 'custom_uptime_ranges'=>$days,];
  $time = time() - urpro_data("refresh","no");
  $key = 'cache-'.$id;
  $check = $wpdb->get_results ( "SELECT * FROM $table_name WHERE ur_key = '$key' AND time > '$time'");
  if(count($check) == 0){
...
function urpro_api_monitorsloop($offset,$limit){
			...
			$loopdata = ['api_key' => urpro_data("apikey","no"), 'limit'=>$limit, 'offset'=>$offset,];
		...
		curl_setopt($curl, CURLOPT_POSTFIELDS, http_build_query($loopdata));
...
function urpro_api_monitorlist(){
 ...
 if(urpro_datatime('monitorlist','no') < $refreshtime or count(json_decode(urpro_data('monitorlist','no'),true)) == 0){
		...
		$limit = "45";
		$loopdata = ['api_key' => urpro_data("apikey","no"), 'limit'=>$limit,];
